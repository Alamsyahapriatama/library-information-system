...
export default function LayananMultimediaPage() { // Renamed component
  const [memberId, setMemberId] = useState(''); // No. Anggota
  const [fullName, setFullName] = useState(''); // Nama Lengkap
  const [kelas, setKelas] = useState(''); // Kelas
  const [serviceUsed, setServiceUsed] = useState(''); // Layanan Yang Digunakan (new field)
  const [officerName, setOfficerName] = useState(''); // Petugas (new field)
  ...
  const loggedInEmail = "alamsyah.bahati@gmail.com"; 
  ...
  const handleSubmit = async (e) => {
    ...
    if (!memberId || !fullName || !kelas || !serviceUsed || !officerName) {
    ...
    // In a real application, you would send this data to your library's multimedia service logging API.
    ...
    try {
      ...
      const response = await fetch('/api/submit-multimedia-service', { // Replace with your actual backend API URL
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-100 pt-28 pb-12 flex justify-center items-start">
      ...
      <div className="max-w-2xl w-full bg-white rounded-xl shadow-lg p-8 space-y-6">
        ...
        {/* Descriptive text for Layanan Multimedia */}
        <p className="text-gray-700 text-base leading-relaxed bg-blue-50 p-4 rounded-lg border border-blue-200">
          Layanan Multimedia adalah layanan yang secara langsung bersentuhan dengan teknologi informasi
          yang di miliki oleh perpustakaan. Koleksi yang termasuk dalam layanan multimedia adalah tablet,
          komputer, peralatan presentasi, (infocus) dan televisi.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
            ...
            <div className="mt-2 space-y-2">
              {['Komputer', 'Tablet', 'Tv', 'Proyektor'].map((service) => (
                <div key={service} className="flex items-center">
                  <input
                    ...
                    required={!serviceUsed} // Only required if no option is selected yet
                  />
                  <label htmlFor={`service${service}`} className="ml-2 block text-base text-gray-700">
                    {service}
                  </label>
                </div>
              ))}
          ...
          {/* Petugas */}
          <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
            <label htmlFor="officerName" className="block text-lg font-medium text-gray-900 mb-2">
          ...
          {submissionStatus === 'success' && (
            <div className="flex items-center p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
              ...
              <div>
                <span className="font-medium">Berhasil!</span> Data penggunaan layanan multimedia Anda telah terkirim. Terima kasih!
              </div>
            </div>
          )}
